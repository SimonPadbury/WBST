...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h4>
              <em>
                <span class="text-muted author"><?php _e('By', 'wbst'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'wbst' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'wbst' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="glyphicon glyphicon-folder-open"></i>&nbsp; <?php _e('Category', 'wbst'); ?>: <?php the_category(', ') ?><br/>
...
<?php if ( function_exists('wbst_pagination') ) { wbst_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
